...
2. Нақты сәйкестік (exact match) — тақырып пен сипаттамадан
3. Fuzzy match — `thefuzz` + `python-Levenshtein` арқылы (ұқсас атаулар)
4. Нәтижелер `confidence` белгісімен қайтарылады: `'exact'` немесе `'fuzzy'`
...
- `get_tariff_keyboard(callback_prefix)` → inline батырмалар (`buy_tariff:` немесе `gift_tariff:` prefix)
- `get_tariff_description(tariff_id)` → форматталған мәтін (invoice-та қолданылады)
