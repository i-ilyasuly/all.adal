    ...
    if "photo" in msg:
        ...
        if text == "⚙️ Баптаулар":
            ...
            tariff_text = (
                ...
                "Халал өнім табылса 🎉 конфетти эффекті, мерзімі өткен болса 👎 реакция — нәтижені бірден байқайсыз.\n\n"
                "━━━━━━━━━━━━━━━━━━\n"
            ...
            if is_symbat:
                ...
                if not current_gender:
                    welcome_text = (
                        f"Сәлем, {first_name}! 👋\n\n"
                        f"Мен — ҚМДБ Халал Даму ботымын. Міне, мен не істей аламын:\n\n"
                        f"🔍 Өнім немесе мекеме атауын жазсаңыз — 1 секундта халал екенін тексеремін\n"
                        f"📸 Өнімнің суретін жіберіңіз — өзім оқып, базадан іздеп беремін\n"
                        f"📍 Тұрған орныңызды жіберіңіз — жаныңыздағы халал мекемелерді тауып беремін\n"
                        f"🗺 Мекемелердің сертификат мерзімін де, мерзімі өткенін де көрсетемін\n"
                        f"🧪 Е-қоспаларды тексере аламын — халал, харам немесе күдікті екенін айтамын\n"
                        f"💬 Кез келген чатта @alladalbot деп жазсаңыз — чаттан шықпай іздей аласыз\n\n"
                        f"Бір сұрақ: сізді қалай атайын? 😊"
                    )
                    gender_markup = {"inline_keyboard": [[
                        {"text": "🧑 Жігіт", "callback_data": "gender:male"},
                        {"text": "👩 Қыз", "callback_data": "gender:female"}
                    ]]}
                    send_message(chat_id, welcome_text, reply_markup=gender_markup, reply_to_message_id=user_msg_id)
                    add_user(chat_id, first_name, username)
                    save_chat_history(chat_id, "user", text)
                    log_to_bigquery(chat_id, "start", "/start", "Жаңа қолданушы (Атауы сұралды)")
                else:
                    welcome_text = f"Қайта келдіңіз, {first_name}! 👋\nТексергіңіз келген нәрсе бар ма? Жазыңыз, суретін жіберіңіз немесе орныңызды жіберіңіз — бірден іздеймін!"
